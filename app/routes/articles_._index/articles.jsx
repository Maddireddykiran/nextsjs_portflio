...
export function Articles() {
  const { posts, featuredPosts } = useLoaderData();
  const [currentFeaturedIndex, setCurrentFeaturedIndex] = useState(0);
  const [currentFeatured, setCurrentFeatured] = useState(featuredPosts?.[0]);
  const featuredRef = useRef(null);
  const { width } = useWindowSize();
  const singleColumnWidth = 1190;
  const isSingleColumn = width <= singleColumnWidth;

  // Effect to cycle through featured posts every 8 seconds
  useEffect(() => {
    if (!featuredPosts || featuredPosts.length <= 1) return;
    
    const interval = setInterval(() => {
      setCurrentFeaturedIndex(prevIndex => {
        const nextIndex = (prevIndex + 1) % featuredPosts.length;
        setCurrentFeatured(featuredPosts[nextIndex]);
        return nextIndex;
      });
    }, 8000); // Change featured post every 8 seconds
  ...
  const featuredPost = currentFeatured ? (
    <Transition
