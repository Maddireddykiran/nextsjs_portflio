...
export const DisplacementSphere = props => {
  ...
  useEffect(() => {
    if (hasWebGLError || !scene.current) return;
  ...
  useEffect(() => {
    if (hasWebGLError || !renderer.current || !camera.current || !sphere.current) return;
  ...
  }, [reduceMotion, windowSize, hasWebGLError]);
  ...
  useEffect(() => {
    if (hasWebGLError || !renderer.current || !scene.current || !camera.current || !sphere.current) return;
  ...
  // If WebGL initialization failed, return a simple gradient background instead
  if (hasWebGLError) {
    return (
      <div 
